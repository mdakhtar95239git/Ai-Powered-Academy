...
export const createCompanion = async (formData: CreateCompanion) => {
    ...
    if (error || !data) throw new Error(error.message || "Failed to create to companions")
...
export const newCompanionPermissions = async () => {
    ...
    else if (has({ plan: '10_companions_limit' })) limit = 10;
    else if (has({ plan: '3_companions_limit' }))  limit = 3;
