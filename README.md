...
- Include `'/__clerk/:path*'` in the proxy matcher so Clerk's auto-proxy path is routed
...
4. Are imports only from `@clerk/nextjs` or `@clerk/nextjs/server`?
5. Is it using App Router, not `_app.tsx` or `pages/`?
6. Is it using `<Show>` instead of `<SignedIn>`/`<SignedOut>`?
